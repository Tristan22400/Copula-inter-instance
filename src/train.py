...
def validate(model: nn.Module, val_files: list, cfg: DictConfig, device: str) -> dict:
    ...
    for batch in val_loader:
        ...
        # Materialise R_hat for structural metrics
        R_hat = torch.bmm(W_tilde, W_tilde.transpose(-2, -1))  # (B, N_max, N_max)
        B = R_hat.shape[0]
        for b in range(B):
            N = batch["n_test"][b].item()
            R_h = R_hat[b, :N, :N].detach()
    ...
    metrics["oracle_frac"] = metrics["copula_nll"] / (metrics["oracle_nll"] + 1e-8)
...
def main(cfg: DictConfig) -> None:
    ...
    # ---- Optimizer + scheduler ----
    optimizer = torch.optim.AdamW(model.parameters(), lr=t.lr, weight_decay=0.0)
    ...
    for step in range(t.steps + 1):
        ...
        with autocast(device_type=device, dtype=amp_dtype, enabled=use_amp):
            W_tilde = model(batch)
            loss = copula_nll(W_tilde, batch["z_test"], batch["test_mask"])
        ...
        if step % t.val_every == 0 and step > 0:
            ...
            print(
                ...
                f"oracle={metrics['oracle_nll']:.4f} "
