...
# real-context predictions collapsed toward ~0 correlation (near-neighbour
# truth 0.97 -> predicted 0.05-0.13). This is a train/deploy distribution-shift
# problem, not a lengthscale-prior or evaluation-methodology problem (both
# were ruled out).
#
...
def _generate_gp_batch_raw(
    ...
    # Built as plain dense kernel + diagonal nugget (kernel_obj(x_kernel).to_dense()
    ...
    if raw_y_override:
        ...
        # module above. Every currently-registered backend has one, so this
        # is defensive rather than reachable in production -- it is what a
        # newly-added backend gets before its own _batched module exists
        # (and what offline callers can legitimately stay on, since the
        # per-episode cost only matters for live generation). Unlike the
        # batched branch
