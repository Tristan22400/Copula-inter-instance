...
def _generate_gp_batch_raw(
    ...
    # rescaling needed.  Filter degenerate episodes instead. Near-singular K_ff
    # can blow alpha/K_inv_diag up to inf, and inf * 0 (from rsqrt of an
    # inf-clamped K_inv_diag) is NaN — `~isfinite` catches that; a plain
    # `z_std < 0.1` comparison would not, since NaN fails every comparison.
    z_std = z_train.std(dim=1)
    degen = ~torch.isfinite(z_std) | (z_std < 0.1) | (z_std > 3.0)
    if degen.any():
        warnings.warn(
