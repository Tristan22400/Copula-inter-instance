...
def extract_model_true_z_train_correlation(
    ...
    `context_coords`/`coords_test` are z-scored via
    ``inference.copula_inference.normalize_features`` (joint train+test
    stats), matching extract_model_context_correlation's sibling call --
    synthetic mode's `coords_test` is a fixed, known-in-advance grid, same
    as the real-data branch.
    """
    from inference.copula_inference import normalize_features
